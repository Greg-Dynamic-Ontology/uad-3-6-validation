"""Acceptance test for IT-25R5S1 pending-cycle findings outcome."""
...
def _pending_cycle_outcome_contract() -> tuple[Any, type[Any]]:
    """Load the first-result lifecycle contract required by IT-25R5S1."""
    ...
    assert callable(apply_result), (
        "IT-25R5S1 requires apply_first_actionable_validation_result("
        "validation_cycle_id, validation_result, clock, repository, "
...
def test_it_25_r5_s1_opens_pending_cycle_when_findings_exist() -> None:
    """Open the cycle and publish that billable service was delivered."""

    apply_result, pending_cycle = _pending_cycle_outcome_contract()
    ...
    transition_time = datetime(2026, 8, 27, 18, 0, tzinfo=timezone.utc)
    cycle = pending_cycle(
    ...
    result = ActionableValidationResult(
        ...
        findings=(
            ValidationFinding(
                ...
                message="A required postal code is missing.",
            ),
        ),
    )
